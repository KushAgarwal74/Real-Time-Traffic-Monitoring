        ...
        try:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    # table to store breakdown of vehicle types for each event
                    cur.execute(
                        """
                    ...
                    cur.execute(
                        """
                        SELECT create_hypertable('traffic_breakdown', 'event_time', if_not_exists => TRUE);
                        """
                    )
                    cur.execute(
                        """
                        CREATE INDEX IF NOT EXISTS idx_traffic_breakdown_time
                        ON traffic_breakdown (event_time DESC);
                        """
                    )
                conn.commit()
            return
